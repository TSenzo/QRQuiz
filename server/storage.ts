...
export class MemStorage implements IStorage {
  ...
  async createQuiz(insertQuiz: InsertQuiz): Promise<Quiz> {
    ...
    // Transformer questions en tableau si nécessaire
    const questionsRaw = Array.isArray(insertQuiz.questions)
    ? insertQuiz.questions
    : Object.values(insertQuiz.questions || {});
  
  // Valider et parser les questions via Zod pour lever une erreur si invalides
  const questions: Question[] = questionsRaw.map((q) => questionSchema.parse(q));
    const quiz: Quiz = { 
      id,
